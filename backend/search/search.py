...
==============================================================
main.py가 실제로 쓰는 건 이 파일의 chunk_text() 하나뿐이다(섹션 헤더 우선 청킹,
문서 인덱싱 시 사용). BM25Indexer는 [정리 2026-07-28] Elasticsearch 마이그레이션으로
main.py의 호출부(_get_search_indexer)가 제거되면서 더 이상 쓰이지 않는다 — BM25는
이제 ES 내장 Lucene BM25(+ standard 분석기, nori 플러그인은 아직 미설치)가 담당한다.
kiwipiepy 기반 한국어 형태소 토크나이저가 필요해지면(예: nori 설치가 계속 막힐 경우
ES 앞단에서 직접 토큰화해 넘기는 대안) 이 클래스를 다시 쓸 수 있어 일단 남겨둔다.
